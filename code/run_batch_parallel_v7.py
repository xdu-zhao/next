...
def run_one_combo(
    ...
    # Graph/operator text can contain characters outside the Windows ANSI
    # code page.  Logs must never make an otherwise valid search fail.
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=int,
        help="Explicit case IDs to run; overrides --start-case/--end-case.",
    )
    parser.add_argument(
        "--problems",
        nargs="+",
    ...
    parser.add_argument(
        "--solver",
        default="code/solve_multicore_v7.py",
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    case_ids = (
        ...
        if args.case_ids
        else range(args.start_case, args.end_case + 1)
    )
    for i in case_ids:
