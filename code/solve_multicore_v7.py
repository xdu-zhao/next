...
def candidate_plans(
    ...
    if problem == 1:
        ...
        if mode == "none":
            ...
            # Five-core screening portfolio.  These points cover the main
            # partition boundaries without paying the full-grid cost.
            wave_values = [
                0.55,
                0.75,
                0.95,
                ...
                1.5,
                2.0,
                # Full-search regressions showed that five-core P1 often
                # needs a finer task wave: 2.5/3/4 repaired every large
                # quick-vs-incumbent loss observed in the first pass.
                2.5,
                3.0,
                4.0,
                # Cases that still select 4.0 have a monotonic improvement
                # trend, so the refinement runner also probes this sparse
                # high-wave tail before declaring the partition saturated.
                5.0,
                6.0,
                8.0,
                10.0,
            ...
            # V8 portfolio: unite the complementary V6 and V7 probes.
            # P1 plateaued under micro-tuning around 0.6, so these points
            # test different component grouping boundaries.
            wave_values = [
                0.30,
                0.35,
                0.45,
    ...
    if mode == "quick":
        # Coarse partitions are frequent five-core winners; probe both sides
        # of w=1 while keeping the screening pass compact.
        for coarse_w in (0.75, 0.95, 1.25):
            yield (
                f"coarse-w{coarse_w:g}",
                plan_scene_a(graph, num_cores, waves=coarse_w),
            )
    ...
    if mode == "full":
        ...
        for coarse_w in [
            ...
            1.25,
            1.50,
            # V7 dense coarse-w1 neighborhood.
            # Retain V6's complementary coarse boundaries as well.
            0.55,
            0.70,
            0.72,
            0.75,
    ...
    cluster_params = (
        ...
        if mode == "quick"
            ...
            (0.22, 0.62),
            # V7 selected points.
            # Lower cluster weights test whether V5 sometimes over-clusters.
            (0.10, 0.12),
            (0.14, 0.18),
            (0.18, 0.22),
            (0.22, 0.28),
            # Higher-beta points test stronger pipe/load balancing.
            (0.26, 0.35),
            (0.30, 0.42),
            # V6 off-diagonal probes complement the V7 portfolio: they
            # independently vary load balancing and communication clustering.
            (0.08, 0.35),
            (0.08, 0.50),
            (0.12, 0.20),
            (0.12, 0.50),
    ...
    if problem == 3 and mode in ("quick", "full"):
        # V5: preserve V4 values and search around the winning .55/.80 region.
        # V8: fine low/mid search plus V6's high-value tail.
        cache_values = (
            # Case 027 selected the former high-end value 1.6, so retain a
            # sparse tail probe instead of assuming the cache preference
            # saturates in the quick portfolio.
            (0.20, 0.55, 0.80, 2.0, 2.5, 3.5)
            if mode == "quick"
            else (
                0.20, 0.28,
                0.35, 0.40, 0.45, 0.50, 0.55,
                0.60, 0.65, 0.72, 0.80, 0.88, 0.95,
                1.10, 1.30, 1.60, 2.0, 2.5, 3.5,
            )
        )
        for cv in cache_values:
            yield (
        ...
        # V6 P3 cross-search: independently vary cluster affinity and
        # shared-cache value, retaining a useful complementary family.
        cross_values = [] if mode == "quick" else [
            (0.15, 0.55),
            (0.15, 0.80),
            (0.45, 0.55),
            (0.45, 0.80),
        ...
        for cscale, cv in cross_values:
            yield (
                f"cache-cluster-rank-c={cscale:g}-cv={cv:g}",
                plan_cluster_rank_affinity(
                    graph, num_cores, beta=0.12,
                    comm_scale=comm_scale, input_scale=input_scale,
                    cluster_scale=cscale, cache_value_scale=cv,
...
def plan_signature(plan):
    """Stable identity for removing equivalent search candidates."""
    return json.dumps(
        plan,
        sort_keys=True,
        separators=(",", ":"),
...
def main():
    ...
    for index, (
        ...
        if signature in seen_plans:
            print(
                f"[{index:02d}] {name}: SKIP duplicate plan"
            )
            continue
